...
-- The Book workspace owns generic identity and delegates optional settings to
-- the pack that defines them. The reporting asset shown here is read-only;
-- changing an established ledger's denomination is a separate operation.
...
AS $$
	...
	SELECT
	    ...
	    books.reporting_asset,
	    books.entity_type,
	    presentation.text('entity.' || entity_types.id)
...
UNION ALL
	...
	jsonb_build_object(
	    'page', 'book',
	    'book_id', p_book_id,
	    'book_exists', book.book_id IS NOT NULL,
	    'archived', book.archived_at IS NOT NULL
	) || CASE
	    WHEN book.book_id IS NULL THEN jsonb_build_object(
		'configuration_status', 'ordinary',
		'validation_messages', jsonb_build_array('Book does not exist.')
	    )
	    ELSE api.uk_book_status(book.book_id)
	END
    FROM (VALUES (TRUE)) AS singleton(only_row)
    LEFT JOIN book ON singleton.only_row
UNION ALL
    SELECT
	...
	jsonb_build_object(
	    'id', book.book_id,
	    'name', book.book_name,
	    'reporting_asset', book.reporting_asset,
	    'entity_type', book.entity_type,
	    'entity_type_label', book.entity_type_label,
...
UNION ALL
    ...
    FROM public.book_entity_types AS entity_types
    WHERE EXISTS (SELECT 1 FROM book)
UNION ALL
    SELECT
	'reporting_currency'::VARCHAR,
	20000 + row_number() OVER (ORDER BY history.effective_from DESC),
	history.effective_from::TEXT,
	jsonb_build_object(
	    'effective_from', history.effective_from,
	    'asset', history.asset,
	    'current', history.asset = book.reporting_asset
    ...
    JOIN book ON book.book_id = history.book_id
UNION ALL
    SELECT
    ...
    WHERE EXISTS (SELECT 1 FROM book)
UNION ALL
    SELECT * FROM api.uk_book_components(p_book_id)
UNION ALL
